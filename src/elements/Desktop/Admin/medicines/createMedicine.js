...
class CreateMedicine extends React.Component {
    ...
    componentDidMount() {
        ...
        if (url.pathname === '/addMedicineToMachine') {
            ...
            fetch(sessionStorage.getItem('machineIP') + "/medicines?prescription=both", {
                headers: headers,
            })
        ...
        this.slotIput = document.querySelector('#slotID');
    ...
    validateForm() {
        if (this.state.pathname) {
            ...
            if (this.state.slotID.trim() === '') {
                this.setError(this.slotIput, 'Provide slot id');
            } else {
                this.setSuccess(this.slotIput)
            }
        }
